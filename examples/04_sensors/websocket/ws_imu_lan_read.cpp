...
Run:
    ...
    .\build\Release\aidog_ws_imu_lan_read.exe --bind 0.0.0.0 --port 8766 --hz 50
Expected result:
    The host prints IMU yaw, pitch, and roll after the robot connects.
Exit:
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        nlohmann::json config{{"cmd", "sensor_stream"}, {"imu", {{"enable", true}, {"hz", options.hz}}}};
        host.send_control_json(config);
        std::cout << "requested IMU stream hz=" << options.hz << "\n";
        ...
        if (options.stopStreamOnExit) {
            try {
                nlohmann::json stopConfig{{"cmd", "sensor_stream"}, {"imu", {{"enable", false}}}};
