...
This document records the protocol details currently exposed by the C++ SDK.
...
Config writes use the `ae01` characteristic and send a compact UTF-8 JSON object terminated by `\0`.
...
IMU angles may be reported by firmware as milli-degrees. The SDK normalizes them into `yawDeg`, `pitchDeg`, and `rollDeg`, while preserving the raw payload for advanced users.

## WebSocket Mapping

WebSocket text frames wrap BLE-compatible payloads:

- `control_raw`: raw packet equivalent to BLE `ae03`.
- `config_json`: config JSON equivalent to BLE `ae01`.
- Sensor JSON frames are fed into the same parser used by BLE notifications.
- Binary frames are raw PCM for bidirectional audio.
